...
if systemLang in data['system']:
    ...
    startupText = data["system"][systemLang]["sentences"]["startupText"].format(day=bugun, month=ay, year=now.year)
    url = f"https://{systemLang}.wikipedia.org/wiki/{bugun}_{ay}"
    inputText = data["system"][systemLang]["sentences"]["inputText"].format(confirm=data["system"][systemLang]["sentences"]["confirm"].lower())
